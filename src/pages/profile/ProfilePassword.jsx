...
import {postAsyncPasswordProfile, profileClearResult} from "../../feature/redux/ProfileSlice.jsx";


const ProfilePassword = ({open_close,reload,open_slider}) => {
    ...
    const {result,isLoading} = useSelector(state => state.profile);
    // redux
    ...
    const validationSchema = yup.object({
        old_password: yup
            .string()
            .required("رمز عبور قبلی خود وارد کنید")
    ...
    const onSubmit = (values) => {
            dispatch(postAsyncPasswordProfile(values ));
    };
                    ...
                    <div className={`relative md:max-w-2xl w-full rounded-tr-4xl dark:shadow-gray-600 rounded-bl-4xl shadow-lg bg-gray-50 dark:bg-gray-800 transform transition-all duration-300 ease-in-out p-4 pt-1 ${isOpenModal ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'} z-10`}>
                        ...
                        <form onSubmit={formik.handleSubmit}>
                            <div className="flex flex-col gap-6 p-5 pb-5 text-blue-950 ">
                                <Input formik={formik} name="old_password" type="password" maxLength={20}  minLength={8} label="رمز عیور قبلی" noPersian={true} />
                                <Input formik={formik} name="new_password" type="password" maxLength={20}  minLength={8} label="رمز عبور جدید" noPersian={true} />
                                <Input formik={formik} name="password_check" type="password" maxLength={20}  minLength={8} label="رمز عبور جدید" noPersian={true} />
