...
const AddBrand = ({Id,list_brand,open_close,reload,open_slider}) => {
    ...
    const {result,isLoading} = useSelector(state => state.brand);
    // redux
    const foundItem = list_brand?.find(item => item.id === Id);
    ...
    const onSubmit = (values) => {
        if (Id) {
            dispatch(postAsyncEditBrand(values));
        } else {
            dispatch(postAsyncAddBrand(values));
        }
    };
                        ...
                        <form onSubmit={formik.handleSubmit} className="bg-gray-50 dark:bg-gray-800 rounded-3xl p-2 space-y-5">
                            <div className="flex flex-col md:flex-row md:gap-4 gap-6">
                                ...
                                <div className="w-full flex flex-col items-center justify-center gap-10">
                                    <Input formik={formik} maxLength={30} name="title" label="نام برند" />
                                    <Input formik={formik} maxLength={50} name="url" noPersian={true} label="url" />
                                    {!foundItem && (
                                        <SelectOption
                                            formik={formik}
