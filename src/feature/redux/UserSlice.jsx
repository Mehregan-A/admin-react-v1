...
export const getAsyncListUser = createAsyncThunk("user/getAsyncListUser",async (payload,{rejectWithValue})=>{
    try {
        const res = await http.get(`admin/user/list/${payload.row}/${payload.page}`,{})
        return await res
    }catch (error) {
...
const userSlice = createSlice({
    ...
    extraReducers : (builder)=>{
        builder.addCase(getAsyncListUser.fulfilled,(state, action)=>{
            state.list_user = action.payload.data.result
            state.isLoading_list = false
            state.isError_list = false
        })
        builder.addCase(getAsyncListUser.pending,(state)=>{
            state.list_user = false
            state.isLoading_list = true
            state.isError_list = false
        })
        builder.addCase(getAsyncListUser.rejected,(state,action)=>{
...
export const { userClearResult,userClearInfo,userClearResultDelete} = userSlice.actions
